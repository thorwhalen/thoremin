...
export function buildSystemPrompt(): string {
  return [
    ...
    '- The overlay, the hand mapping and the expression map are STRUCTURED dials — they hold an',
    '  object, so they have no single value. Change one value inside them with',
    '  dial.setIn({ path, value }), where path is a dotted leaf path (e.g. "overlay.landmarks.show",',
    '  "handMap.fingers.index.target", "faceExpr.degrees.happy"). The tool lists every valid path.',
