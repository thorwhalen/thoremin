 ...
 * so the UI must say so rather than toast an unqualified "Saved" (#143). */
export interface RecordingResult extends SinkResult {
   ...
   *  planned and always written: with (say) FLAC as the only selected audio format and the
...
export class SessionRecorder {
   ...
   * a `.flac` holding WebM bytes is a file the player only discovers is broken much
   * later, so the take reports the failure instead (#143).
   */
