...
| **Body** (#186) — the pose source + slot, the body feature catalog (angles, kinematics, shape, Laban effort, relations, rhythm), the dance pulse behind the `PulseEngine` seam (`PULSE_HOLD_CONFIDENCE`), body → sound routing | `src/nodes/sources/{webcam_body,synthetic_body,replay_body,body_contract}.ts`, `src/features/body_catalog.ts`, `src/nodes/features/{body_feature_vector,body_pulse,pulse_engine}.ts`, `src/nodes/mapping/{body_map,body_route}.ts`, `src/app/bodyRouteDemand.ts`, `docs/research/body-and-pace-research-map.md` |
