...
| **Sub-frame impact prediction** — the impact predictor (a strike predicted before the frame that shows it: quadratic approach extrapolated to a known or learned plane, commit-then-confirm), the timing magnet (actuality ↔ intent dial), the trend prior (tempo-rate `RhythmPrior`); the `an.impacts` scoring harness and its numbers | `src/ictus/impact.ts`, `magnet.ts`, `trend_prior.ts`; `scripts/subframe/` (clip sets, scorer, fixture builder; see its `README.md`); `test/subframe/`, `test/fixtures/subframe_*`; [`docs/research/subframe-impact-prediction.md`](docs/research/subframe-impact-prediction.md) |
