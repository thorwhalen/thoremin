...
| **5** (after 4) | `commands/steer.ts` (the scalar `steer.strain.*` domain commands, §6d) + `VibeEditor` ported onto them; a keyboard shortcut for the transport. Until then `steerConfig`'s strain arrays are a dial only a future editor writes (its scalar leaves are already `dial.setIn`-able). | jsdom | no |
