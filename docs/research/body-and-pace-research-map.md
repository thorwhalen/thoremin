...
8. **D-Jogger is the design precedent for "music follows the mover", and it is quantified.** Moens et al. [R5] tested four strategies on runners: tempo-matching alone raised phase coherence to R = 0.17, starting in phase plus tempo-matching to R = 0.73, and additionally nudging tempo whenever phase drifted past ±30° to **R = 0.92**. Their stretch range was ±10 % (a phase vocoder); beyond it they changed song. Van Dyck et al. [R6] found the spontaneous entrainment basin for runners is about ±2.5 %. For this epic the rule is: match period, start in phase, correct phase by nudging rate, clamp to ±10 % by default (widen as a dial), and never jump the audio.
9. **Kinematic beats are velocity minima; visual beats are decelerations.** AIST++ [R7] defines a dance's kinematic beats as local minima of joint speed and scores alignment with a Gaussian of the beat distance (σ = 0.06-0.12 s); `muvid` already ports that scorer (`motion_beat_bas`). Davis & Agrawala [R8] define visual beats from the deceleration ("impact") envelope of motion. Pedersoli & Goto [R9] learned beats from OpenPose keypoints alone with a TCN and reached F ≈ 0.65-0.70 — a ceiling to remember: pose alone will not give an audio-grade beat, which is exactly why the prior (`ictus`) is needed.
...
| MoveNet Lightning (TF.js) | 17 (COCO) | `@tensorflow/tfjs-*` **already shipped** for the legacy hand model + `@tensorflow-models/pose-detection` | 21 KB gz [measured] | ~3 MB int8 (model card) | Apache-2.0 | 34 fps browser; 50+ fps laptop [R14] | fallback seam only; no world landmarks, no z |
...
- It is a **`source` slot candidate**, not an always-on node. `?slot.source=webcam-body` swaps it in; the hands source swaps out. A player who wants both hands and body pays for both knowingly, later, through an explicit multi-source graph — not by default.
...
| **Beat This!** [R17] via `onnxruntime-web` | beats + downbeats, per-beat confidence | model 97 MB fp32 (20 M params); a **2 M-param small variant** exists with comparable F1 (~8 MB fp32, ~2 MB int8) **[from docs]**; `onnxruntime-web` wasm ≈ 10-20 MB at runtime | MIT / MIT | browser (WebGPU/WASM) | beat F1 89.1, downbeat 78.3 on GTZAN, without DBN **[from docs]** | the upgrade candidate for downbeats; needs an ONNX export of the small model and a browser mel front-end |
...
| `kodokan.segment.estimate_period` / paces `acf_periods` (top-k) | period(s) from ACF | the pulse estimator (§4.2), causal windowed | to build |
...
| `muvid.footage.scoring.motionbeat.motion_beat_bas` | per-beat Gaussian alignment score | the R / BAS score in the replay test | to build (test only) |
...
The dial: `'body.enabled'` (boolean, facet `Body`) and `'body.model'` (`lite` | `full`), added to `src/settings/dials.ts` and its two bijections, emitted by `store-controls` on a `bodyEnabled` port, and wired `ui.bodyEnabled → camBody.enabled` with a structural guard in `test/app_graph.test.ts` (the #147 template: an unconnected `enabled` is the exact failure the shipping rule exists for). Cold-load clicks to "select the body source": open the Body panel, one toggle. Two.
...
Nothing in the repo plays an audio file (`playbackRate` has zero hits). The song state — decoded `AudioBuffer`, its `SongGrid`, `playing`, `follow` — is added to the hot store (`src/app/store.ts`) so `store-controls` can emit it on a `song` port each tick (an object reference per tick, as `chordConfig` already is); this avoids touching `useEngine`'s resource bag. Loading is a shell tool: `song` in `src/app/tools.ts` (kind `panel`, hotkey), a `SongPanel.tsx` with a file input that decodes via `decodeAudioData`, runs the §5.3 tracker in a Worker (`src/song/beatTrack.ts`, pure, unit-tested on synthetic clicks) and writes the store; `test/tools_shell.test.tsx` / `test/app_shell.test.ts` prove it is reachable. Cold-load clicks to "load a song": tool button, file picker. Two.

`song-player` (`src/nodes/output/song_player.ts`, `BROWSER_NODES`, roles `synth`) consumes `song` and a live `rate` port, owns a `Stretcher` (`src/song/stretcher.ts`: `media-element` default via `createMediaElementSource` into `ctx.resources.masterGain`; `signalsmith` as a lazy `import('signalsmith-stretch')` candidate), and is a no-op without `audioContext` — the `webaudio_synth.ts` guard. Dials: `'song.follow'`, `'song.rateMin'`, `'song.rateMax'`, `'song.stretcher'`.
...
Catalog: `webcam-body`, `synthetic-body`, `replay-body`, `body-feature-vector`, `body-pulse`, `body-route` file under a new **Body** section of `scripts/gen_catalog.ts`'s `CATEGORIES`; `song-player` and `pace-controller` under a new **Song** section (the catalog test fails on the "Other" catch-all). `test/app_graph.test.ts`'s node count moves from 18 to 23 on purpose.
...
[R17] Foscarin F, Schlüter J, Widmer G. Beat this! Accurate beat tracking without DBN postprocessing. In: *Proc. ISMIR*. 2024. [https://arxiv.org/abs/2407.21658](https://arxiv.org/abs/2407.21658); ONNX port with model sizes [https://github.com/mosynthkey/beat_this_cpp](https://github.com/mosynthkey/beat_this_cpp)
