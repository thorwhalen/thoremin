...
| Personal Orchestra [8] | 2002 to 04 | Buchla Lightning II IR baton | Tempo, volume, section emphasis | Sign change of dy/dt (lowest point) | Per-beat ratio, catch-up window | 93% of visitors controlled tempo |
| You're the Conductor [9] | 2004 | IR baton | Tempo from speed, volume from size | **None** (decoupled from beats) | Velocity magnitude → tempo | Children's museum; audio stretch |
...
| You're the Conductor | 2003 to 04 | IR baton, children | gesture speed → tempo, no beat sync; exponential decay | modified phase vocoder | WAITING state | [148] |
| iSymphony / Maestro! (PhaVoRIT) | 2006 | baton; adaptive pattern | beat-synchronised for conductors, speed-only for novices | phase vocoder, rate update 43/s, ≤69 ms | pause supported | [12,149,180] |
...
| UBS Virtual Maestro | 2007 to 09 | Wii accelerometer | IBI of velocity peaks, recursively smoothed | playback rate | no motion → stop | [150] |
...
`src/nodes/output/orchestra.ts` is a `synth`-role node whose `init()` lazily imports `spessasynth_lib`, adds its worklet module, and fetches the self-hosted orchestral SF3 (§5.4) into the Cache API on first use; it exposes a `status` port (`off | loading | ready | error`, the same shape as `midi-out`) so the UI can show progress and the structural guard can assert the enable input is connected. It consumes scheduler commands (`noteOn/noteOff/cc` with times) and the conductor's `dynamics` as CC11 per section with a per-section low-pass; sections map to channels. `src/app/pacer/` (later, shared with #186) is the `AudioPacer`: a lazily imported `signalsmith-stretch` worklet, the recording's beat grid, and the servo of §6.7 evaluated at the audio output timestamp.
...
4. **Scheduler = pending-event with a short commit horizon and a Δt servo**, HOLD as a state with fermata support, preparatory-beat start (§6.3, §7.3). Firing at the predicted time, never waiting for the ictus.
...
6. **Orchestra = `spessasynth_lib` in an AudioWorklet plus a self-hosted orchestral subset SF3 cut from MuseScore_General (MIT)**, under 15 MB, cached on first use; CC11 plus a low-pass for continuous dynamics; VSCO-2-CE with CC1 crossfades as the premium path (§5.4).
...
[12] Lee E, Karrer T, Borchers J. Toward a Framework for Interactive Systems to Conduct Digital Audio and Video Streams. Computer Music Journal. 2006;30(1):21–36. [DOI 10.1162/014892606776021317](https://doi.org/10.1162/014892606776021317).
...
[54] Dannenberg RB. An on-line algorithm for real-time accompaniment. In: Proc. ICMC 1984, Paris; p. 193-198. [Michigan Publishing archive](https://quod.lib.umich.edu/i/icmc/bbp2372.1984.025/1) (page returns 403 to automated fetch; located via search)
...
[148] Lee E, Nakra TM, Borchers J. [You're the Conductor: A Realistic Interactive Conducting System for Children](https://www.nime.org/proceedings/2004/nime2004_068.pdf). Proc. NIME 2004, pp. 68–73.

[149] RWTH Media Computing Group. [Personal Orchestra project page](https://hci.rwth-aachen.de/po).

[150] Nakra TM, Ivanov Y, Smaragdis P, Ault C. [The UBS Virtual Maestro: an Interactive Conducting System](https://www.nime.org/proceedings/2009/nime2009_250.pdf). Proc. NIME 2009, pp. 250–255.

[151] Todd N. [A Model of Expressive Timing in Tonal Music](https://continuum-hypothesis.com/music/todd.pdf). Music Perception 1985;3(1):33–57.

[152] Todd NPM. [The kinematics of musical expression](https://pubs.aip.org/asa/jasa/article-abstract/97/3/1940/838457/). J. Acoust. Soc. Am. 1995;97(3):1940–1949 (the "1992" model; JASA 1992;91(6):3540 is "The dynamics of dynamics"). (fetch blocked)

[153] Friberg A, Sundberg J. [Does music performance allude to locomotion? A model of final ritardandi derived from measurements of stopping runners](https://pubs.aip.org/asa/jasa/article-abstract/105/3/1469/558501/). J. Acoust. Soc. Am. 1999;105(3):1469–1484. (fetch blocked)

[154] Friberg A, Bresin R, Sundberg J. [Overview of the KTH rule system for musical performance](https://continuum-hypothesis.com/music/kth.pdf). Advances in Cognitive Psychology 2006;2(2–3):145–161.

[155] Friberg A, Colombo V, Frydén L, Sundberg J. Director Musices: The KTH Performance Rules System. ([DiVA PDF](https://www.diva-portal.org/smash/get/diva2:1246181/FULLTEXT01.pdf) — connection failed during check; unverified)

[156] Sullivan J. [Alternatives to Lookahead Audio Scheduling](https://repository.gatech.edu/entities/publication/ac112129-16e6-44b3-872b-ed063a32a5df). Web Audio Conference 2016, Atlanta.

[157] MDN. [AudioContext: getOutputTimestamp() method](https://developer.mozilla.org/en-US/docs/Web/API/AudioContext/getOutputTimestamp).

[158] Tone.js. [Tone/core/clock/Transport.ts](https://github.com/Tonejs/Tone.js/blob/dev/Tone/core/clock/Transport.ts) (dev branch).

[159] Tone.js. [Tone/core/clock/TickParam.ts](https://github.com/Tonejs/Tone.js/blob/dev/Tone/core/clock/TickParam.ts).

[160] Tone.js. [Tone/core/clock/TickSource.ts](https://github.com/Tonejs/Tone.js/blob/dev/Tone/core/clock/TickSource.ts).

[161] MDN. [HTMLMediaElement: preservesPitch property](https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/preservesPitch).

[162] Chromium. [media/filters/audio_renderer_algorithm.cc](https://chromium.googlesource.com/chromium/src/+/main/media/filters/audio_renderer_algorithm.cc) (main).

[163] Chromium. [third_party/blink/renderer/core/html/media/html_media_element.h](https://chromium.googlesource.com/chromium/src/+/main/third_party/blink/renderer/core/html/media/html_media_element.h) (`kMinPlaybackRate = 0.0625`, `kMaxPlaybackRate = 16.0`).

[164] blink-dev. [Intent to Implement and Ship: HTMLMediaElement.preservesPitch](https://groups.google.com/a/chromium.org/g/blink-dev/c/XVCt0EXVplQ/m/sDyHmylZAgAJ). June 2020.

[165] Mozilla Bugzilla. [Bug 495040 — Implement playbackRate and related bits](https://bugzilla.mozilla.org/show_bug.cgi?id=495040).

[166] WebKit. [MediaSessionManagerCocoa.mm — audioTimePitchAlgorithmForMediaPlayerPitchCorrectionAlgorithm](https://github.com/WebKit/WebKit/blob/main/Source/WebCore/platform/audio/cocoa/MediaSessionManagerCocoa.mm).

[167] WebKit Bugzilla. [Bug 240405 — preservesPitch=false and playbackRate aren't correctly handled when hooked up to AudioContext](https://bugs.webkit.org/show_bug.cgi?id=240405) (fixed 2026-03-27).

[168] MDN. [AudioBufferSourceNode: playbackRate property](https://developer.mozilla.org/en-US/docs/Web/API/AudioBufferSourceNode/playbackRate).

[169] Signalsmith Audio. [Signalsmith Stretch — Web Audio release (README in npm `signalsmith-stretch`)](https://github.com/Signalsmith-Audio/signalsmith-stretch).

[170] Signalsmith Audio. [Signalsmith Stretch (C++ library page)](https://signalsmith-audio.co.uk/code/stretch/).

[171] cutterbl. [SoundTouchJS monorepo](https://github.com/cutterbl/SoundTouchJS).

[172] npm. [@soundtouchjs/audio-worklet README](https://unpkg.com/@soundtouchjs/audio-worklet@2.1.1/README.md).

[173] Daninet. [rubberband-wasm](https://github.com/Daninet/rubberband-wasm).

[174] Breakfast Quay. [Rubber Band Library licensing](https://breakfastquay.com/rubberband/license.html).

[175] delude88. [rubberband-web](https://github.com/delude88/rubberband-web).

[176] Superpowered. [Web Audio JavaScript/WebAssembly SDK](https://github.com/superpoweredSDK/web-audio-javascript-webassembly-SDK-interactive-audio).

[177] Superpowered. [Pricing](https://superpowered.com/pricing).

[178] Correya A, et al. [Audio and Music Analysis on the Web using Essentia.js](https://transactions.ismir.net/articles/10.5334/tismir.111). TISMIR 2021.

[179] Lee E, Karrer T, Borchers J. [An Analysis of Startup and Dynamic Latency in Phase Vocoder-Based Time-Stretching Algorithms](https://hci.rwth-aachen.de/publications/lee2007f.pdf). Proc. ICMC 2007.

[180] Karrer T, Lee E, Borchers J. [PhaVoRIT: A Phase Vocoder for Real-Time Interactive Time-Stretching](https://hci.rwth-aachen.de/publications/karrer2006a.pdf). Proc. ICMC 2006, pp. 708–715.

[181] Ishizaki H, Hoashi K, Takishima Y. [Full-Automatic DJ Mixing System with Optimal Tempo Adjustment based on Measurement Function of User Discomfort](https://archives.ismir.net/ismir2009/paper/000043.pdf). Proc. ISMIR 2009, pp. 135–140.

[182] Stark AM, Davies MEP, Plumbley MD. [Real-Time Beat-Synchronous Analysis of Musical Audio](https://dafx.de/paper-archive/details.php?id=XiEJjIc0a2Hb_KTJon0SaA). Proc. DAFx 2009; implementation: [BTrack](https://github.com/adamstark/BTrack).
