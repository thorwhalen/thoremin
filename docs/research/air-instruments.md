...
- **Drums are the timing problem in its purest form**, and on real footage (§7.3) the pose model's wrists see about half of a roll's hits and reverse about 50 ms after the sound: the stick and fingers move, the wrist does not, so the anchor source must be the hand landmarks or the stick tip. Dahl's air-drumming measurement is the paper the sub-frame stream should read first: at 200 fps the velocity reversal (the "hit") comes *after* the audio onset and drifts with tempo (−3 ms fast, +44 ms slow), while acceleration peaks come *before* it and hold steady, so trigger on the acceleration peak [B2]. That is the conducting map's peak-deceleration ictus, measured on drummers.
- **There is no camera-based air flute in the literature, and this footage shows why.** Breath is sensed with microphones [C4, C5], the mouth with a head-worn camera in 2003 [C2], the face with optic flow in 2005 [C3]. On 31,000 labelled frames (§7.2) flute fingering from both hands' landmarks is at chance across players and 27 to 45% even with ten seconds of the player's own footage per note: a key press is under the tracker's angular noise floor edge-on. An air flute needs large enrolled finger lifts; the embouchure blendshapes are the newer sensor, and the onset question is a timing measurement now possible on local data.
- **Bass pitch is not a shape** (§7.1): 10% across players and 32% within, because a pitch is a fret and a string while the fretting hand looks the same at every fret. An air bass maps a displacement along an imaginary neck to pitch, the theremin's own mapping, and its plucking hand is a rhythm anchor.
...
Seven videos, five players, tracked for hands and face (a fixed camera on a flautist gives the tracker 65 to 87% of frames with a hand and, when the player faces the camera, 97 to 100% with a face; the play-along source had no person in frame and was dropped). Labels: the *fingering class* of the sounding note from `label_pitch.py` (probabilistic YIN with a per-source tuning offset, range C4 to C6): the pitch class over the first two octaves, which share their fingerings on a Boehm flute, except D5 and D#5, which are played with the left index lifted and keep their own classes; the third octave, fingered differently again, is out of range. Fourteen classes, 31,217 frames over four players after the join (both hands required). Features: both hands' chord-shape vectors, prefixed by side, plus twenty embouchure blendshapes (`mouthPucker`, `mouthFunnel`, `jawOpen`, the cheek and lip-press shapes), 100 in all.

| held-out player | frames | accuracy | macro-F1 | accuracy, 9-frame vote |
|---|---|---|---|---|
| lance-suzuki (three videos) | 21,600 | 3.8% | 2.2% | 3.6% |
| lauren-teaches-flute | 3,343 | 7.5% | 2.9% | 7.7% |
| musicians-addition | 5,753 | 33.7% | 14.4% | 34.3% |
| selfridge (embouchure clip, two notes) | 521 | 0.0% | 0.0% | 0.0% |
| **pooled, softmax** | 31,217 | **9.6%** | 6.1% | 9.6% |

Chance for fourteen classes is 7%. Hands only, without the face, scores the same within a point (a pitch-class variant without the D5/D#5 split gave 11.5% hands-only against 12.3% with the face): the embouchure neither helps nor hurts a fingering model. The within-player random split reaches 70.3%, which looks like separability until the enrolment experiment is run on the same data: with ten seconds of their own footage per class, the two players who cover all fourteen classes reach 27.1% and 44.7% (softmax; 21.0% and 34.9% with the trainer's centroid), and the beginner who plays a handful of notes 57.5%. Adding the other players' frames to that enrolment moves it by a few points either way (21.1% and 51.9%). That is the number that matters, and it says the within-player 70% is mostly adjacent frames leaking across a random split, not fingering being read.
...
Eight YouTube sources (five real, three air) and the maintainer's own 2021 air-drum clip, tracked with the pose model (33 landmarks). The stroke detector is not a new one: `lib_drum_strokes.ts` feeds each wrist's shoulder-normalised image position to the conductor's `IctusDetector` (`src/ictus/detector.ts`), so a drum stroke and a conductor's beat are one definition, with two settings changed for drums (no median-of-three prefilter, which flattens the five-frame cycle of a 6 Hz roll and biases the timing by half a frame; a shorter initial period and refractory fraction). On synthetic wrists it finds every off-frame stroke to within a millisecond and every stroke of a continuous 2, 4 or 6 Hz groove. Labels: hit onsets from the audio (`label_onsets.py`), 658 to 2,455 per real source. Scoring: the beat-tracking F-measure (`src/ictus/metrics.ts`) at one frame and at 70 ms, raw and after removing the median stroke-to-onset lag, which is reported because it is the number the sub-frame stream has to predict away. Assignment: each wrist's landing points clustered by recursive bisection (split only a wide cluster, only when it helps, only into halves of at least four strokes).

| source (player) | fps | body found | strokes L/R | onsets | lag ms | F at 1 frame, raw / lag-corrected | F at 70 ms, raw / lag-corrected | drums found L+R |
|---|---|---|---|---|---|---|---|---|
| bTuZkz09Crg (atl-drum-academy), single-stroke roll on a pad | 60 | 96% | 296/229 | 658 | −28 | 6.4% / 10.0% | 33.5% / 34.2% | 1+1 |
| LCVliBvczG8 (chris-johnson), full kit, basic grooves | 30 | 99% | 610/610 | 817 | +51 | 12.1% / 34.3% | 36.8% / 47.1% | 1+1 |
| QRBzyirReLs (adhdrums), single-stroke roll at 85 bpm | 30 | 47% | 1/25 | 1,030 | | 2.3% | 3.4% | 1+3 |
| H3SoYdS31Ik (sight-read-drums), rudiments with a notation overlay | 25 | 0% | 0/0 | 785 | | | | none |
| WlmRf8alZb4 (alejandro-sifuentes), singles at three subdivisions | 24 | 63% | 507/612 | 2,455 | +8 | 24.1% / 24.3% | 33.2% / 33.6% | 1+1 |
| FqJdzYY_Fas (william-wei), air | 24 | 100% | 320/361 | | | | | 1+1 |
| YmhdvtCTfJI (andrew-wk), air | 24 | 100% | 382/271 | | | | | 4+5 |
| R8Z6deMf_3c (coop3rdrumm3r), air, a medley with cuts | 30 | 94% | 113/140 | | | | | 5+5 |
| the maintainer's own clip, air, 60 fps | 60 | 100% | 236/230 | | | | | 5+1 |

Three findings, none of them about the detector.

**The wrist is the wrong point.** On every real source the wrists yield far fewer strokes than the audio has hits (525 against 658 on the pad roll; 1,119 against 2,455 on the subdivided singles), and the best lag-corrected F-measure is 47% at 70 ms. A drummer's roll is played from the fingers and the rebound: the stick tip travels, the wrist barely moves, so a wrist-level turning point sees the accented strokes and misses the rest. The two sources with poor body detection (a sheet-music overlay across the frame; a drummer half out of frame) add nothing. The next tracker is the hand landmarks (twenty-one points, the fingertips) or the stick tip itself, both of which the pipeline already reaches (`extract.py --streams hands`); the pose stream is the right one for the body and the wrong one for the stick.

**The reversal trails the sound.** Where the match is good enough to measure, the wrist reverses 51 ms *after* the onset on the 30 fps kit and 8 ms after on the 24 fps singles; on the 60 fps pad roll it leads by 28 ms, which at that frame rate is the accent stroke's rebound being read as the turning point. Dahl's measurement [B2] is the frame of reference: at 200 fps the reversal trails the onset by up to 44 ms and the acceleration peak leads it by 14 to 33 ms. A frame-level detector cannot be the instrument, exactly as §5.3 of the timing report says; this is the measurement of how far behind it is on real footage, and the lag column is what the sub-frame stream's braking-onset predictor has to recover.

**Drums do not separate in wrist space.** On the full kit the recursive bisection finds one cluster per wrist: snare, hi-hat and toms are reached by the stick and the forearm with the wrist sweeping a small arc, so the landing points form one cloud. The air sources find four to five clusters per wrist because an air drummer moves the whole arm, which is the one encouraging number in the table: the air player's "drums" are where their wrists go, and those are separable. The maintainer's own clip shows the asymmetry of a right-handed air drummer, five places for the left wrist and one for the right.

**Where the two streams meet.** This module ends at a `Stroke`, which *is* an `Anchor` from `src/ictus/types.ts` plus a wrist and a landing point. Everything downstream of the anchor (the rhythm prior, the predicted next hit, the commit horizon, the sub-frame refinement from the braking profile) is `src/ictus/**` and the sub-frame stream's. What this stream adds on its side of the seam is the drum assignment, and the evidence above about which point to track. The air-drum plan, then: hand landmarks rather than the pose wrists as the anchor source; enrolment of the player's own "drums" as landing-point clusters (the trainer again, over positions this time); and the instrument sound scheduled by the sub-frame stream's predictor, not by the anchor.

## 8. What stays local and what is committed

Committed: the scripts, their tests on synthetic hands and wrists, the source lists, this document with its numbers. Local under the app-data dir, never committed: the videos and their metadata, every landmark stream, every label file, every dataset, model and result file. The tests cannot depend on the local data and do not; the CI gate (`npm run typecheck`, `npm test`) is green without any footage on the machine. When a fixture is wanted for the app (a replay of a chord change through the `hand-feature-vector` node, say), it is recorded from the maintainer's own hand with the existing `npm run record` path, not cut from YouTube.
