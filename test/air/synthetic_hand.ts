 ...
 * curl per finger and one spread per adjacent-finger pair, rendered into pixel
 * coordinates at a chosen position, scale and in-plane rotation. Not a hand model;
 * enough that the catalog's joint angles, spreads and pinch distances take distinct,
 * controllable values per "chord shape" and stay put under translation, scaling and
 * rotation, which is what the feature and model tests need.
 *
...
function chain(base: [number, number], dir: number, curl: number, segs: readonly number[], scale: number): [number, number][] {
  const pts: [number, number][] = [];
  let [x, y] = base;
  let a = dir;
  for (const len of segs) {
    x += Math.cos(a) * len * scale;
    y += Math.sin(a) * len * scale;
    pts.push([x, y]);
    a += curl;
  }
  return pts;
...
/** Build the 21 keypoints (pixels, z = 0) of a hand in a given shape and pose. */
export function syntheticHand(shape: HandShape, pose: HandPose, handedness: 'Left' | 'Right' = 'Left'): Hand {
  const s = pose.scale;
  const up = -Math.PI / 2;
  const local: Record<number, [number, number]> = {};
  local[LM.wrist] = [0, 0.5 * s];
  const mcpX = { index: -0.3, middle: -0.1, ring: 0.1, pinky: 0.3 } as const;
  const fingerBase = (f: keyof typeof mcpX): [number, number] => [mcpX[f] * s, -0.25 * s];
  const fingerIdx = {
  ...
  for (const f of ['index', 'middle', 'ring', 'pinky'] as const) {
    const base = fingerBase(f);
    local[fingerIdx[f][0]] = base;
    const pts = chain(base, up + shape.spread[f], shape.curl[f], FINGER_SEGMENTS, s);
    pts.forEach((p, i) => (local[fingerIdx[f][i + 1]] = p));
  }
  const thumbBase: [number, number] = [-0.45 * s, 0.2 * s];
  local[LM.thumb_cmc] = thumbBase;
  const tpts = chain(thumbBase, up - 0.9, shape.curl.thumb, THUMB_SEGMENTS, s);
  ...
  for (let i = 0; i < 21; i++) {
    const [lx, ly] = local[i];
    const [rx, ry] = rot(lx, ly, pose.rotation ?? 0);
    keypoints.push({ x: pose.cx + rx, y: pose.cy + ry, z: 0 });
  }
  return { handedness, keypoints, score: 0.95 };
}
