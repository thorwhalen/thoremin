...
of that strategy (``impact_onsets.ts`` or ``air_drum_hits.ts`` output); ``--strategy
truth`` sounds the exact impacts, the ideal. Onsets that carry ``predicted`` and
``velocity`` (the air drum's hits) are drawn filled when predicted ahead of the strike
and hollow red when not (a ghost note, or a prediction made too late to schedule, which
the node reports as late), and sounded at their velocity. ``--speed 0.25`` renders slow motion: each camera frame is held
...
GHOST = (60, 60, 230)  # BGR, a hit the node could not schedule ahead (ghost or late)
SOUNDS = {'wood': (180, 0.03), 'kick': (70, 0.08)}  # body Hz, decay s


def drum(seed=0, *, sound='wood'):
    """A short, dry hit: a pitched decay plus a noise click."""
    rng = np.random.default_rng(seed)
    freq, decay = SOUNDS[sound]
    n = int(0.25 * SR)
    t = np.arange(n) / SR
    body = np.sin(2 * np.pi * freq * t * (1 - 0.3 * t)) * np.exp(-t / decay)
    click = rng.normal(0, 1, n) * np.exp(-t / 0.004)
    return 0.6 * body + 0.25 * click


def audio_for(onsets, t0, duration, speed, *, velocities=None, sound='wood'):
    track = np.zeros(int(SR * duration / speed) + SR)
    hit = drum(sound=sound)
    for k, t in enumerate(onsets):
        if t0 <= t < t0 + duration:
            i = int(SR * (t - t0) / speed)
            v = velocities[k] if velocities else 1.0
            track[i : i + len(hit)] += v * hit[: len(track) - i]
    return track[: int(SR * duration / speed)]


def draw_strip(img, now, *, frames_t, impacts, grids, onsets, color, ghosts=()):
    h, w = img.shape[:2]
    y0 = h - STRIP_H
    ...
    for k, t in enumerate(onsets or []):
        if visible(t):
            x = x_of(t)
            ghost = k in ghosts
            cv2.drawMarker(img, (x, y0 + 40), GHOST if ghost else color, cv2.MARKER_TRIANGLE_UP, 16, 1 if ghost else 3)
...
def main():
    ...
    ap.add_argument('--strategy', default=None, help='truth, or a key of the onsets file')
    ap.add_argument('--title', default=None, help='overlay title (default: the strategy label)')
    ap.add_argument('--sound', default='wood', choices=list(SOUNDS))
    ...
    if a.strategy == 'truth':
        ...
        onsets = None
    hits = on['onsets'].get(a.strategy, []) if a.strategy and a.strategy != 'truth' else []
    ghosts = {k for k, o in enumerate(hits) if o.get('predicted') is False}
    velocities = [o['velocity'] for o in hits] if hits and 'velocity' in hits[0] else None
    color = COLORS.get(a.strategy or 'truth', COLORS['predicted'])
    ...
    for k in range(n_out):
        ...
        if onsets:
            for k, t in enumerate(onsets):
                c = GHOST if k in ghosts else color
                if 0 <= now - t < max(0.06 * a.speed, a.speed / OUT_FPS):
                    cv2.rectangle(img, (0, 0), (W - 1, H0 - 1), c, 6)
                if a.strategy != 'truth' and 0 <= now - t < 0.35 * max(1, a.speed * 2):
                    err = 1000 * (t - min(impacts, key=lambda x: abs(x - t)))
                    txt = f'{abs(err):.0f} ms ' + ('late' if err > 0 else 'early') + (' (not ahead)' if k in ghosts else '')
                    cv2.putText(img, txt, (10, H0 - 16), cv2.FONT_HERSHEY_SIMPLEX, 0.8, c, 2, cv2.LINE_AA)
        draw_strip(img, now, frames_t=frames_t, impacts=impacts, grids=grids, onsets=onsets, color=color, ghosts=ghosts)
        title = a.title or LABELS.get(a.strategy, a.strategy or 'ground truth') + (f'   x{a.speed:g} speed' if a.speed != 1 else '')
    ...
    if onsets is not None:
        track = audio_for(onsets, a.start, duration, a.speed, velocities=velocities, sound=a.sound)
